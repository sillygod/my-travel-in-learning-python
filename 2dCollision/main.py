...
class Circle:
    ...
    def isCollision(self, circle):
        ...
        if v.length() < self.radius+circle.radius:
            ...
            circle.pos = self.pos + v.normalize()*(self.radius+circle.radius)
...
class Enviromment:
    ...
    def present(self, hdc):
        for obj in self.ptList:
            pygame.draw.circle(hdc, (255, 255, 255), obj.boundCircle.pos(), obj.boundCircle.radius, 0)
...
class App:
    ...
    def __init__(self, width, height):
        ...
        self.hWndDC = pygame.display.set_mode((width, height), self.fullScreen, 32)
        self.winInfo = pygame.display.Info()
        self.selectedObj = None

        self.env = Enviromment()
        self.env.boundregion = (0, 0, self.winInfo.current_w, self.winInfo.current_h)
    ...
    def start(self):
        self.timer = pygame.time.Clock() # it seems that it will start the timer when it be created
        while True:
            self.timer.tick(30)
            pygame.display.set_caption('{}'.format(round(self.timer.get_fps(), 2)))
            self.eventQueue()
            self.update()
            self.present()
